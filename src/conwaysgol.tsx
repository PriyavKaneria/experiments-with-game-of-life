...
const updateCell = (cell: number, alive: number) => {
	...
	if (cell === STATES) {
		// Parse survival rule
		if (isNumberInSet(alive, SURVIVAL)) return STATES // survive
		return STATES - 1 // underpopulation or overpopulation
	}
	// dead
	else {
		// Parse birth rule
		if (isNumberInSet(alive, BIRTH)) return STATES // birth
		return 0 // stay dead
	}
}
